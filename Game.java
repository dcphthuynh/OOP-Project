...
public class Game extends Canvas implements Runnable {
	...
	private final Window window = new Window(WIDTH, HEIGHT, "Candy Crush -  game by team Amateur Coder!", this);
	...
	public Game() {
		...
		// Main menu
		PlainImage mainLogo = new PlainImage(TEXTURE_MANAGER.getTexture("logo_main.png"), (WIDTH - 400) / 2, 50, 400,
				400);
		candycrush.Button exitButton = new candycrush.Button("Exit", Color.WHITE, 45, (WIDTH - 250) / 2, 625, 250, 75,
				TEXTURE_MANAGER.getTexture("button_pink_long.png"));
		candycrush.Button playButton = new candycrush.Button("Play", Color.WHITE, 50, (WIDTH - 250) / 2, 500, 250, 100,
				TEXTURE_MANAGER.getTexture("button_yellow_long.png"));
		...
		// Levels selector
		candycrush.Button backToMenu = new candycrush.Button((WIDTH / 2) - 50, 800, 100, 100,
				TEXTURE_MANAGER.getTexture("back_to_menu.png"));
		candycrush.Button levelPageLeft = new Button((WIDTH / 2) - 170, 800, 100, 100,
				TEXTURE_MANAGER.getTexture("arrow_pink_left.png"));
		candycrush.Button levelPageRight = new candycrush.Button((WIDTH / 2) + 70, 800, 100, 100,
				TEXTURE_MANAGER.getTexture("arrow_pink_right.png"));
		...
		for (int i = 0; i < pageCount; i++) {
			...
			for (int j = i * 24; j < (i + 1) * 24 && j < LEVEL_MANAGER.getNumberOfLevel(); j++) {
				...
				candycrush.Button temp = new candycrush.Button("" + levels.get(j).getNumber(), Color.WHITE, 30,
						x * (WIDTH / 7) - 50, y * (HEIGHT / 6) - 50, 100, 100,
						TEXTURE_MANAGER.getTexture("square_button.png")) {
					@Override
					public void tick() {
						super.tick();
		...
		candycrush.Button backToLevelSelector = new candycrush.Button(10, HEIGHT - 110, 100, 100,
				TEXTURE_MANAGER.getTexture("back_to_menu.png")) {
			@Override
			public void tick() {
				super.tick();
				if (currentLevel != null) {
					if (currentLevel.isWin()) {
						int result = JOptionPane.showConfirmDialog(window, "You Won !!! Go to the next level ?",
								"Message", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
						if (result == JOptionPane.YES_OPTION) {
							currentLevel = LEVEL_MANAGER.getLevels().get(currentLevel.getNumber()); // get currentLevel
							grid.setUpGrid(currentLevel);
							LEVEL_HANDLER.setLevel(currentLevel); //set Next Level
